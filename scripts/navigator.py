...
from planners import AStar, compute_smoothed_traj, GeometricRRTStar
...
class Navigator:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/dilated_map", OccupancyGrid, self.map_callback)
    ...
    def replan(self):
        ...
        # problem_rrtstar = GeometricRRTStar(
        #     state_min,
        #     state_max,
        #     x_init,
        #     x_goal,
        ...
        # do RRTStar for max_rrt_iter times, and if fails, move to AStar
        max_rrt_iter = 0
        i = 0
        success_rrtstar = False
        while i < max_rrt_iter and not success_rrtstar:
        	print("RRT")
        	success_rrtstar = problem_rrtstar.solve(0.1)
        	i += 1
        
        if success_rrtstar:
        	pass
        	# rospy.loginfo("RRTStar Planning Succeeded")
        	# planned_path = problem_rrtstar.path
        else:
        	rospy.loginfo("RRTStar Planning Failed")	
        	success_astar = problem_astar.solve()
        	if not success_astar:
		        rospy.loginfo("AStar Planning Failed")
		        return
        	rospy.loginfo("AStarPlanning Succeeded")
