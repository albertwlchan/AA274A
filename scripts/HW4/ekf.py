...
class EkfLocalization(Ekf):
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        H_list = []
        Hs = np.array(Hs)
        pred_covs = np.einsum('ijk,ilk->ijl', (Hs @ self.Sigma), Hs)
        for i in range(I):
            z = z_raw[:, i]
            Q = Q_raw[i]
            ...
            # compute innovations
            innovs = np.empty((J, 2))
            innovs[:, 0] = angle_diff(z[0], hs[0, :])
            innovs[:, 1] = z[1] - hs[1, :]

            # compute Mahalanobis distances
            innov_covs = pred_covs + Q
            Mahalanobis_distances = np.einsum('ij,ij->i',
            				       innovs,
            				       np.einsum('ijk,ik->ij', np.linalg.inv(innov_covs), innovs))
            j_min = Mahalanobis_distances.argmin()
            # import pdb; pdb.set_trace()
            if Mahalanobis_distances[j_min] < self.g ** 2:
                v_list.append(innovs[j_min])
                Q_list.append(Q)
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g[:3], Gx[:3, :3], Gu[:3, :3] = tb.compute_dynamics(self.x[:3], u, dt, compute_jacobians = True)
        ########## Code ends here ##########
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx_tilde = tb.transform_line_to_scanner_frame(np.array([alpha,r]), self.x[:3], self.tf_base_to_camera, compute_jacobian=True)
           
            Hx[:,0:3] = Hx_tilde
           
            if j >= 2:
                x_cam_base, y_cam_base, th_cam_base = self.tf_base_to_camera
                x_base, y_base, th_base = self.x[:3] #pose of base in world frame
   		  ...
   		  [0, 0, 1]])
                ...
                h11 = 1
                h12 = 0 
                h21 = (np.sin(alpha)*camx - np.cos(alpha)*camy) 
                h22 = 1
                Hx[:,idx_j:idx_j+2] = np.array([[h11,h12],[h21,h22]]) # FIX ME!
            ########## Code ends here ##########
